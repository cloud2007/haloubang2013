...
$t = time();
$t = strtotime("-31 days", $t);
$str.="<dataset seriesName='过去30天发布出租房源情况' color='0033CC' anchorBorderColor='0033CC'>";
for($i=0;$i<30;$i++){
	$t = strtotime("+1 days", $t);
	$ts = date("Y-m-d", $t);
	$house = new House();
	$num = $house ->count(array('whereAnd'=>array(array('edittime','>'.$t),array('edittime','<'.($t+86400),array('type','=1'),array('borker_id','='.$_SESSION['borker_id'])))));
	$str.="<set value='".$num."' toolText='".$ts.":".$num."套' />";
}
$str.="</dataset>";

$str.="<dataset seriesName='过去30天发布出售房源情况' color='FF0000' anchorBorderColor='FF0000'>";
for($i=0;$i<30;$i++){
	$t = strtotime("+1 days", $t);
	$ts = date("Y-m-d", $t);
	$house = new House();
	$num = $house ->count(array('whereAnd'=>array(array('edittime','>'.$t),array('edittime','<'.($t+86400),array('type','=2'),array('borker_id','='.$_SESSION['borker_id'])))));
	$str.="<set value='".$num."' toolText='".$ts.":".$num."套' />";
}
$str.="</dataset>";
