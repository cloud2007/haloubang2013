...
try {
    ...
    ShowMsg('你查看的房源不存在！现在为你跳转到房源列表','/sale',0,3000);exit();
}
...
$likehouse = $houses -> find(
        array(
            'whereAnd'=>array(array('type','=2'),array('b_id','='.$borough->id)),
            'limit'=>'0,4',
            'order'=>'rand',
        )
);
...
$postion = '<a href="/">首页</a> > <a href="/sale">出售房源</a> > '.$house->b_name.$house->h_floor.'F'.$house->roomnum.'号';
$view -> set ('postion',$postion);

$view -> set('web_title',"成都{$borough->b_name}{$house->h_floor}楼{$house->roomnum}号{$house->area}平米写字楼出售 - 好楼帮");
$view -> set('web_keywords',"{$borough->b_name},{$borough->b_name}写字楼,{$borough->b_name}出租信息,成都写字楼出售");
