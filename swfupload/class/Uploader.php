...
class Uploader {
    ...
    function __construct() {
        ...
        $this->maxsize = 2097152; //2097152;//最大上传大小，默认是2M
        $this->msgtype = 1;//返回上传参数的格式：1，只返回url，2，返回参数数组
        $this->allowFile = 'txt,rar,zip,jpg,jpeg,gif,png,swf,wmv,avi,wma,mp3,mid'; //上传扩展名
        /*try {
            $this->enableSize = Config::item('borough.borough');
			$this->enableSize = Config::item('borough.'.$cfgfile);
        } catch (Exception $e) {
            exit($e->getMessage());
        }*/
    }
                   ...
                   switch ($this->dirtype) {
                    ...
                    if ($this->needResize == true) {//需要生成缩略图
                        foreach ($this->enableSize as $size => $val) {
							//echo $this->attachdir.$target;die;
    ...
    function resize($imagePath, $size) {  
        if ((bool)$imagePath && is_file(Url::filePath($imagePath)) && in_array($size, array_keys($this->enableSize))) {
            ...
            $image->resize();
			if ($this->needWaterMark && $size=='reset'){$this->waterMarkImage(str_replace($this->attachdir,'',$image->full_dst_path));}//reset图片添加水印（原图已有水印）
            $image->clear();            
        }
    }
    
    private function waterMark($imagePath, $config) {
        $file = BASEDIR . UPLOAD_PATH. "{$imagePath}";
...
//                'wm_font_path' => Url::filePath('msyh.ttf'),//水印字体名字和路径                
    ...
    private function waterMarkImage($imagePath) {
        ...
        $this->waterMark($imagePath, array(
            'wm_overlay_path' => '../images/ava.png',//水印图像的名字和路径       
